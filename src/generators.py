...
def filter_by_currency(transactions_list, currency_code):
    ...
    """
    return list(filter(
            lambda x: x.get('operationAmount', {}).get('currency',
            {}).get('code') == currency_code, transactions_list)
    )


def transaction_descriptions(transactions_list, descriptions_code=None):
